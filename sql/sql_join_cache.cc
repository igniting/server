...
uint JOIN_CACHE::write_record_data(uchar * link, bool *is_full)
{
  ...
  *is_full= last_record;
  ...
  { 
    ...
    if (!top_level_cond_is_satisfied(join_tab->on_precond, thd()))
    {
...
DBUG_RETURN(!top_level_cond_is_satisfied(join_tab->cache_idx_cond, thd()));
}
...
bool JOIN_CACHE_BKAH::skip_index_tuple(range_id_t range_info)
{
  ...
  do
  {
    ...
    uchar *rec_ptr= next_rec_ref_ptr + rec_fields_offset;
    get_record_by_pos(rec_ptr);
    if (top_level_cond_is_satisfied(join_tab->cache_idx_cond, thd()))
      DBUG_RETURN(FALSE);
  } while(next_rec_ref_ptr != last_rec_ref_ptr);
  DBUG_RETURN(TRUE);
}
